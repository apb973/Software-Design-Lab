...
public class Queryable {
	...
	public class Query extends AsyncTask<String, Void, Document>{

		@Override
		protected Document doInBackground(String... url) {
			Document doc = null;
			try{
				doc = Jsoup.connect(String.valueOf(url[0])).get();
				return doc;
			}catch(Exception e){
				return doc;
			}
	...
	public Station specificGoogleStation(String address, Location location){
		...
		Query query = new Query();
		try {
			doc = query.execute("https://maps.googleapis.com/maps/api/place/nearbysearch/xml?location=" + location.getLattitude() + ",+" + location.getLongitude() + "&rankby=distance&type=gas_station&key=AIzaSyDvb8h33wTteNR1NbHN9f0m1Y2HfgdwkwU&keyword" + address).get();
			Elements nameElements = doc.select("name");
			Elements locElements = doc.select("location");
			Elements latElements = locElements.select("lat");
			Elements longElements = locElements.select("lng");
	...
	public GasQueryInput gasPriceQueryInfo(Location location){
		...
		Query query = new Query();
		try {
			doc = query.execute("https://maps.googleapis.com/maps/api/place/nearbysearch/xml?location=" + location.getLattitude() + ",+" + location.getLongitude() + "&radius=500&type=restaurant&key=AIzaSyDvb8h33wTteNR1NbHN9f0m1Y2HfgdwkwU").get();
			Elements nameElements = doc.select("reference");
			String id = nameElements.get(0).text();
			query = new Query();
			doc = query.execute("https://maps.googleapis.com/maps/api/place/details/xml?reference=" + id + "&key=AIzaSyDvb8h33wTteNR1NbHN9f0m1Y2HfgdwkwU").get();
	...
	public Restaurants googleRestaurants(Location location){
		...
		Query query = new Query();
		try {
			doc = query.execute("https://maps.googleapis.com/maps/api/place/nearbysearch/xml?location=" + location.getLattitude() + ",+" + location.getLongitude() + "&radius=400&type=restaurant&key=AIzaSyDvb8h33wTteNR1NbHN9f0m1Y2HfgdwkwU").get();
			Elements nameElements = doc.select("name");
	...
	public Results FindGasPrice(int zipcode, String city){
		...
		Query query = new Query();
		try {
			doc = query.execute("http://www.autoblog.com/" + zipcode + "-gas-prices/").get();
